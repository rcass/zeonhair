	...
	function build_css_rule($selector, $style, $mod_name, $prefix = '', $suffix = '', $fonts_only = false, $device = null) {
		...
		if (!empty($mod)) {
			...
			if ('logo' === $style || 'tagline' === $style) {
				...
				if (isset($bg->style) && '' != $bg->style && 'noimage' !== $bg->noimage ) {
					$out .= 'fullcover' === $bg->style ? "\n\tbackground-size: cover;" : "\n\tbackground-repeat: {$bg->style};";
				}
				if (isset($bg->position) && '' != $bg->position && 'noimage' !== $bg->noimage ) {
					$out .= "\n\tbackground-position: {$bg->position};";
				}
				if( isset($bg->fixedbg) && 'fixed' === $bg->fixedbg && 'noimage' !== $bg->noimage ) {
