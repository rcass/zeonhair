	...
	var apiResponsiveControl = {
		...
		controlBackground: function( event, model, currentData ) {
			var $transparent = $('.color-transparent', model.container),
				...
				$hideElements = $('.open-media-wrap, .image-label, .background-style, .background-position, .fixed-bg', model.container),
			...
			api(index, function (value) {
				value.bind(function (backgroundData) {
					var values = themifyParseJSON(backgroundData),
							$id = getStyleId(selector),
							$sides = ['image', 'color', 'repeat', 'size', 'position', 'attachment'];
					if (!styles[$id]) {
						styles[$id] = [];
					}
					_.each($sides, function (side) {
					...
					if ('undefined' !== typeof values.style && values.style && 'noimage' !== values.noimage) {
					...
					if ('undefined' !== typeof values.position && values.position && 'noimage' !== values.noimage) {
						styles[$id]['background-position'] = values.position;
					}

					if( typeof values.fixedbg !== 'undefined' && values.fixedbg && 'noimage' !== values.noimage ) {
						styles[$id]['background-attachment'] = values.fixedbg;
					}
