...
class Themify_Builder {
	...
	public function get_builder_data($post_id) {
		...
		$builder_data = stripslashes_deep(themify_maybe_unserialize($builder_data));
...
?>
	...
	public function get_builder_stylesheet( $builder_output ) {
		...
		if( ! $builder_loaded
			&& preg_match( '/themify_builder_row/', $builder_output ) // check if builder has any content
		) {
			$builder_loaded = true;
			...
			if ('zip' == $ext[1] || 'rar' == $ext[1]) {
				...
				if ($wp_filesystem->exists($destination_path . '/builder_data_export.txt')) {
					...
					if (is_serialized($data)) {
						/* serialized string */
						$data = themify_maybe_unserialize($data);
					} else {
